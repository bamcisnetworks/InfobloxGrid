...
namespace BAMCIS.Infoblox.PowerShell.Zone
{
    ...
    public class SetDhcpObjectPSCmd : UpdateObjectPSCmd, IDynamicParameters
    {
         ...
         * PassThru: -PassThru
         * Force: -Force
         * UpdateObject: -Reference -RemoveEmptyProperties [Dynamic Attr Params]
         */
        ...
        public override object GetDynamicParameters()
        {
            ...
            if (this.InputObject == null)
            {
                ...
                string RecordType = this.GetUnboundValue<string>("ZoneType");

                if (!String.IsNullOrEmpty(RecordType))
                {
                    if (Enum.TryParse<InfoBloxObjectsEnum>(RecordType, out base.ObjectType))
                    {
                        foreach (RuntimeDefinedParameter pa in IBXDynamicParameters.ObjectTypeProperties(base.ObjectType, new string[] { _GRID_BY_ATTRIBUTE, _SESSION_BY_ATTRIBUTE, _ENTERED_SESSION_BY_ATTRIBUTE }))
                        {
        ...
        {
            if (base._Force == true || ShouldProcess("DNS Zone Object", "Update", "Updating IBX DNS Zone Object"))
            {
                if (base._Force == true || ShouldContinue("Do you want to update the DNS Zone Object?", "Updating IBX DNS Zone Object"))
                {
                    switch (this.ParameterSetName)
                    {
                        ...
                        case _ENTERED_SESSION_BY_OBJECT:
                            {
                                base.ProcessByUpdatedObject(this.InputObject, this.RemoveEmptyProperties);
                                break;
                            }
                        ...
                        case _ENTERED_SESSION_BY_ATTRIBUTE:
                            {
                                base.ProcessByAttributeForUpdatedObject("ZoneType");
                                break;
                            }
                        default:
                            {
